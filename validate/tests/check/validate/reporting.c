...
GST_START_TEST (test_report_levels)
{
  ...
  fail_unless (g_setenv ("GST_VALIDATE_REPORT_LEVEL",
          "none,pipeline*:monitor,sofake1:all,sofake*__sink:subchain", TRUE));
  runner = gst_validate_runner_new ();
  ...
  monitor =
      (GstValidateMonitor *) g_object_get_data (G_OBJECT (pad),
      "validate-monitor");
  /* But its pad should not as it falls in the sofake*__sink pattern */
  fail_unless (gst_validate_reporter_get_reporting_level (GST_VALIDATE_REPORTER
...
GST_START_TEST (test_specific_levels)
{
  ...
  fail_unless (g_setenv ("GST_VALIDATE_REPORT_LEVEL",
          "synthetic,fakesrc1:subchain,fakesrc2:subchain,funnel*__src*:monitor",
          TRUE));
  runner = gst_validate_runner_new ();
  _create_issues (runner);
